...
# Working dtype for the Newton-Schulz iteration inside ``msign``.
#
# The upstream SSO branch documents (and its commit "DO NOT run `msign` in bfloat16!"
# insists) that this iteration must stay in fp32: it is very sensitive to rounding,
# and bf16 distorts the update direction enough to destabilize training. Later commits
# on that branch ("add bf16 and torch compile", "apply bf16") cast to bf16 anyway for
# speed, leaving the code contradicting its own warning.
#
# Default here is the documented-safe fp32. Set to torch.bfloat16 via
# ``--spectral-ball-msign-dtype bf16`` to reproduce the upstream tip's numerics.
...
def set_msign_dtype(dtype: torch.dtype) -> None:
    """Set the working dtype of the Newton-Schulz iteration in ``msign``."""
    global MSIGN_DTYPE
    if dtype not in (torch.float32, torch.bfloat16):
        raise ValueError(f"msign dtype must be float32 or bfloat16, got {dtype}")
    MSIGN_DTYPE = dtype
...
def _small_msign(G: torch.Tensor, steps: int) -> torch.Tensor:
    ...
    # See MSIGN_DTYPE: fp32 by default because this iteration is rounding-sensitive.
    X = X.to(MSIGN_DTYPE)
...
def power_iteration(w: torch.Tensor, steps: int = 50, eps: float = 1e-20):
    ...
    # Shares MSIGN_DTYPE: upstream flipped this to bf16 for speed at the same time,
    # and σ feeds the retraction, so keep the two consistent.
    w = w.to(MSIGN_DTYPE)
